...
4. `course-campaign` 收尾时输出图片完成度报告，并规范化混排图片尺寸（PNG 截图统一为 `<img width="620">`），确保排版一致。
...
执行 Step 3.5 时，用以下提示词 + 描述注释内容驱动生成：
...
- SVG：已由 `width="620"` 保证，天然一致
- PNG 截图：改用 HTML `<img>` 标签，禁止使用 Markdown `![]()` 语法：
...
`course-campaign` Step E 负责将已有 `![](*.png)` 批量规范化为此格式。
...
扫描所有章节文件，将 Markdown 语法的 PNG 引用替换为统一宽度的 HTML 标签：
...
对每处匹配，改写为：
```html
<img src="原路径" width="620" alt="原 alt 文字" />
```

SVG 引用（`![](*.svg)`）不动，已由 `width="620"` 保证一致。
...
course-generator 写正文
  → Step 3.5 扫描 [流程图]/[对比图]
    → 用 image-protocol 提示词模板生成 SVG
    → open 预览验证
    → A方案插入（注释保留 + 图片引用追加）
  → [截图]/[录屏建议] 原样保留

course-campaign 全部章节完成
  → Step E1：grep 扫描 ![](*.png) → 替换为 <img width="620">
  → Step E2：统计 SVG/截图/录屏 → 输出完成度报告
```
