...
完整的 docx 生成技术规范（依赖安装、标准 imports、主题配色系统 TOOL_THEMES、样式定义、页面设置、列表配置、代码块/提示框样式、引流页 Table、Markdown 同步生成、输出路径与验证）已收录于 references。**编写生成脚本前必须读取 `references/docx-generation-spec.md`**，脚本中的所有样式和主题变量以该文件为准。
...
用平视、具体的语气写，像同行在和朋友解释为什么值得学、怎么快速上手，不摆出站讲台的姿态。第一人称只能承载用户素材或可追溯记录已明确支持的真实经历，不得为了制造“同行感”假写亲历。

❌ 错误示范（AI 腔/教材腔）：
> 本章将系统介绍 XXX 的核心概念与使用方法，帮助读者建立完整的知识体系。

✅ 正确示范（无亲历素材时）：
> 配置 XXX 时，有个容易忽略的细节：参数 Y 一旦写错，启动时就会报 `invalid configuration`。这章先把这个参数说清楚。
...
4. **不得虚构**：易错点必须来自官方文档、可复现验证、用户提供的经历或明确标注的示例。没有亲历证据时，写“常见错误是……”“如果……会……”，禁止写“我当时踩坑了”“我亲测”“我的项目里”。同样不得补造耗时、次数、对话、用户反馈、线上事故或效果数据。
...
- ❌ 把官方文档原文翻译一遍当内容
- ❌ 爹味口吻："你必须"、"切记"——改成说明条件和理由的建议，如"更稳妥的顺序是"、"在 X 情况下建议 Y"
...
| 常见错误 | 每章至少 1 个有来源或可复现的易错点，给出触发条件、具体报错和解决方法 |
...
- [ ] 每章包含可运行代码/命令示例，至少 1 个有来源或可复现的常见错误 + 解决方法
- [ ] 内容不足时已通过 webSearch 调研补充，不靠泛泛描述凑字数
- [ ] 风格自查：无"本章介绍……"开场，无"值得注意的是/不难发现/综上所述"等 AI 腔套话
- [ ] 每章有具体场景锚点开场，有"同行视角"而非"讲师视角"
- [ ] 无爹味口吻（"你必须""切记"），建议已说明适用条件和理由
- [ ] **真实性门禁**：所有第一人称经历、耗时/次数、对话、报错、用户反馈、效果数据和发布结果均有可追溯来源；无来源的内容已改为客观说明、条件句或明确标注的假设示例
