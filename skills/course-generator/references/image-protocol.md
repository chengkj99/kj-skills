...
- **验证**：用 Bash 工具执行 `open <svg路径>` 本地预览，确认渲染正常后再插入正文

## 5. SVG 生成提示词模板

执行 Step 3.5 时，以下内容作为图表生成的执行指令，将 `{描述}` 替换为注释中的实际描述后执行：
...
`course-campaign` Step E1 负责将已有 `![](*.png)` 批量规范化为此格式（Step E1 定义见 course-campaign/SKILL.md）。
